<!-- ╔══════════════════════════════════════════════════╗ -->
<!-- ║         HEADER — animated capsule banner        ║ -->
<!-- ╚══════════════════════════════════════════════════╝ -->
<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0d1117,40:001a33,100:003056&height=260&section=header&text=Aryan%20Dutt&fontSize=72&fontColor=00e5ff&animation=fadeIn&fontAlignY=42&desc=Bioinformatics%20%7C%20Python%20%7C%20NGS%20Data%20Science&descSize=20&descAlignY=62&descColor=7dd3fc" width="100%"/>
</div>

<div align="center">

[![Profile Views](https://komarev.com/ghpvc/?username=biostackaryan&color=00e5ff&style=flat-square&label=PROFILE+VIEWS)](https://github.com/biostackaryan)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/aryan-dutt-4a0986371)
[![Kaggle](https://img.shields.io/badge/Kaggle-20BEFF?style=flat-square&logo=kaggle&logoColor=white)](https://kaggle.com/biostackaryan)
[![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black)](https://huggingface.co/spaces/Biostackaryan/helixmind)
[![Email](https://img.shields.io/badge/Gmail-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:aryann.dutt0@gmail.com)
[![Fiverr](https://img.shields.io/badge/Fiverr-1DBF73?style=for-the-badge&logo=fiverr&logoColor=white)](https://www.fiverr.com/users/biostackaryan/seller_dashboard)
</div>

```
   ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮
5'─╯  ╰─A═T─╯  ╰─G≡C─╯  ╰─T═A─╯  ╰─C≡G─╯  ╰─G≡C─╯  ╰─3'
          │         │         │         │         │
3'─╮  ╭─T═A─╮  ╭─C≡G─╮  ╭─A═T─╮  ╭─G≡C─╮  ╭─C≡G─╮  ╭─5'
   ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯
               ═ 2 H-bonds  ·  ≡ 3 H-bonds
```

## `$ whoami`

```python
aryan = {
    "degree"     : "B.S. Biotechnology",
    "focus"      : ["Bioinformatics", "NGS Analysis", "Data Science"],
    "internship" : "BGTI — NGS · Pathway · Structural Bioinformatics",
    "building"   : "HelixMIND — Integrated Bioinformatics Toolkit",
    "interests"  : ["Sequence Analysis", "ML in Drug Discovery", "Data Visualization"],
    "contact"    : "aryann.dutt0@gmail.com"
}
```

---

## `$ ls ./projects`

### 🧬 HelixMIND — Integrated Bioinformatics Toolkit

[![HelixMIND](https://github-readme-stats-biostackaryan1.vercel.app/api/pin/?username=biostackaryan&repo=helixmind&theme=tokyonight&hide_border=true&bg_color=0d1117)](https://github.com/biostackaryan/helixmind)

> A unified platform to search, annotate, and visualize biological data — wrapping BLAST, Entrez, KEGG, and PDB into streamlined Python workflows.

| Layer | Stack |
|---|---|
| **Interface** | Streamlit · Hugging Face Spaces |
| **APIs** | NCBI Entrez · KEGG · RCSB PDB · PubMed |
| **Core** | Python · Biopython · Pandas · Matplotlib |
| **Status** | 🟢 Active Development |

🔗 **Live Demo →** [huggingface.co/spaces/Biostackaryan/helixmind](https://huggingface.co/spaces/Biostackaryan/helixmind)

```
   ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮
5'─╯  ╰─G≡C─╯  ╰─A═T─╯  ╰─T═A─╯  ╰─C≡G─╯  ╰─A═T─╯  ╰─3'
          │         │         │         │         │
3'─╮  ╭─C≡G─╮  ╭─T═A─╮  ╭─A═T─╮  ╭─G≡C─╮  ╭─T═A─╮  ╭─5'
   ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯
```

## `$ cat rosalind_profile.txt`

<div align="center">

[![Rosalind](https://img.shields.io/badge/ROSALIND-Problem%20Solver-6a0dad?style=for-the-badge&logoColor=white)](https://rosalind.info/users/aryann/)&nbsp;&nbsp;
[![Profile](https://img.shields.io/badge/→%20View%20Profile-rosalind.info%2Fusers%2Faryann-00b4d8?style=for-the-badge)](https://rosalind.info/users/aryann/)

</div>

[Rosalind](https://rosalind.info) is a competitive bioinformatics platform where problems span string algorithms, classical genetics, and applied NGS. All solutions written in Python.

```
  TOPIC                       PROGRESS
  ─────────────────────────────────────────────────────────
  DNA/RNA Manipulation        ████████████░░  String & Complement Ops
  GC Content Analysis         ███████████░░░  Genome Composition
  Motif & Pattern Finding     ██████████░░░░  Substring, Consensus
  Mendelian Genetics          █████████░░░░░  Probability, Punnett
  FASTA Parsing               ████████░░░░░░  Multi-record Handling
  Open Reading Frames         ███████░░░░░░░  Codon Translation, ORFs
  Graph Algorithms            █████░░░░░░░░░  Overlap Graphs, De Bruijn
  ─────────────────────────────────────────────────────────
```

---

## `$ pip list`

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat-square&logo=gnubash&logoColor=white)

**Bioinformatics & Data Science**

![Biopython](https://img.shields.io/badge/Biopython-3776AB?style=flat-square&logo=python&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Matplotlib](https://img.shields.io/badge/Matplotlib-11557c?style=flat-square)
![Seaborn](https://img.shields.io/badge/Seaborn-40C4FF?style=flat-square)

**Databases & Tools**

![BLAST](https://img.shields.io/badge/BLAST-D500F9?style=flat-square)
![NCBI](https://img.shields.io/badge/NCBI_Entrez-00E676?style=flat-square)
![KEGG](https://img.shields.io/badge/KEGG-00ACC1?style=flat-square)
![PDB](https://img.shields.io/badge/RCSB_PDB-FF6D00?style=flat-square)
![PubMed](https://img.shields.io/badge/PubMed-2962FF?style=flat-square)

**DevOps & Deployment**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Streamlit](https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-FFD21E?style=flat-square&logo=huggingface&logoColor=black)

---

## `$ cat experience.log`

**BGTI Internship — Bioinformatics**

```
[✓] Genome databases & NCBI sequence retrieval
[✓] Protein structure analysis & PDB exploration
[✓] KEGG pathway annotation and enrichment
[✓] NGS data workflows and processing pipelines
[→] Ongoing: manuscript-stage project in progress
```

---

## `$ ls ./certifications`

| Certificate | Issuer |
|---|---|
| Python for Everybody Specialization | University of Michigan · Coursera |
| Applied Bioinformatics | Coursera — Bhagesh Sachania |
| Python for Data Visualization | Coursera |

📂 [View all certificates →](https://github.com/biostackaryan/certificates-bioinformatics)

---

## `$ upcoming --publications`

```diff
+ [IN PROGRESS]  ML + Virtual Screening — computational drug discovery study
+ [IN PROGRESS]  Internship-driven bioinformatics analysis paper
+ [IN PROGRESS]  NGS / Data Science analysis report
```

---

## `$ git log --stat`

<div align="center">
  <img src="https://github-readme-stats-biostackaryan1.vercel.app/api?username=biostackaryan&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00e5ff&icon_color=00e5ff&text_color=c9d1d9" width="48%"/>
  <img src="https://streak-stats.demolab.com?user=biostackaryan&theme=tokyonight&hide_border=true&background=0d1117&ring=00e5ff&fire=00e5ff&currStreakLabel=00e5ff" width="48%"/>
</div>

<div align="center">
  <img src="https://github-readme-stats-biostackaryan1.vercel.app/api/top-langs/?username=biostackaryan&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=00e5ff&layout=compact" width="40%"/>
</div>

```
   ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮     ╭──╮
5'─╯  ╰─T═A─╯  ╰─C≡G─╯  ╰─G≡C─╯  ╰─A═T─╯  ╰─T═A─╯  ╰─3'
          │         │         │         │         │
3'─╮  ╭─A═T─╮  ╭─G≡C─╮  ╭─C≡G─╮  ╭─T═A─╮  ╭─A═T─╮  ╭─5'
   ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯     ╰──╯
```

## `$ trophy --show`

<div align="center">

<img src="https://github-profile-trophy-beige-two.vercel.app/?username=biostackaryan&theme=radical&no-bg=true&no-frame=true&row=1&column=6&margin-w=12"/>

</div>

---

## `$ git log --graph` — Contribution Activity

<div align="center">

<img src="https://raw.githubusercontent.com/biostackaryan/biostackaryan/output/github-contribution-grid-snake-dark.svg" alt="contribution snake"/>

</div>

<!--
  NOTE: this image will keep showing as broken until the snake.yml workflow
  (included alongside this file) runs at least once in your repo. It generates
  the "output" branch and the SVG that this <img> tag points to. See the
  workflow file for the one-time setup steps.
-->

---

## `$ cat collaborate.md`

```
▸ Bioinformatics tooling    — CLI tools, automation scripts, API wrappers
▸ NGS pipelines             — variant calling, RNA-seq, genome assembly
▸ ML in biology             — protein function prediction, drug-target interaction
▸ Data visualization        — genomic data, pathway maps, sequence plots
```

Open an issue, submit a PR, or reach out directly.

<!-- FOOTER -->
<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:003056,60:001a33,100:0d1117&height=120&section=footer" width="100%"/>

*⭐ Starring the repositories helps visibility — appreciated.*
</div>
